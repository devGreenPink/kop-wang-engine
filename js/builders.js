...
export function hl(code) {
  ...
  // Use a non-printable private-use-area char as delimiter to avoid digit collision
  const MARK = '\uE000';
  const spans = [];
  function save(html) { spans.push(html); return `${MARK}${spans.length - 1}${MARK}`; }

  // Order matters: longer/more-specific patterns first

  // 1. Full import lines  e.g.  import java.time.LocalDateTime;
  code = code.replace(/\b(import )([\w.]+)(;)/g, (_, kw, pkg, sc) =>
    save(`<span class="tok-kw">${kw}</span>`) +
    save(`<span class="tok-pkg">${pkg}</span>`) + sc);

  // 2. Annotations  @Foo
  code = code.replace(/(@[A-Za-z][\w.]*)/g, m => save(`<span class="tok-ann">${m}</span>`));

  // 3. String literals (HTML-escaped quotes)  &quot;…&quot;
  code = code.replace(/(&quot;[^&\n]*?&quot;)/g, m => save(`<span class="tok-str">${m}</span>`));

  // 4. Keywords
  code = code.replace(/\b(public|private|protected|class|interface|export|extends|implements|null|new|return|void|final|static|abstract)\b/g,
    m => save(`<span class="tok-kw">${m}</span>`));

  // 5. Java types
  code = code.replace(/\b(String|Long|Integer|Double|BigDecimal|Boolean|LocalDateTime|LocalDate|Date|Object|List|Set|Map)\b/g,
    m => save(`<span class="tok-type">${m}</span>`));

  // 6. TS primitive types
  code = code.replace(/\b(string|number|boolean|any)\b/g,
    m => save(`<span class="tok-type">${m}</span>`));

  // 7. Line comments
  code = code.replace(/(\/\/[^\n]*)/g, m => save(`<span class="tok-cmt">${m}</span>`));

  // 8. Standalone numbers (NOT inside a saved token placeholder)
  code = code.replace(/(?<![A-Za-z_\uE000])(\d+)(?![A-Za-z_\uE000])/g,
    m => save(`<span class="tok-num">${m}</span>`));

  // Restore all saved spans
  code = code.replace(new RegExp(`${MARK}(\\d+)${MARK}`, 'g'), (_, i) => spans[+i]);
  return code;
}
...
function buildTsColumnDecorator(row) {
  ...
  // Length (string types)
  const effLen = row.length !== null ? row.length : (row.anns.columnLength !== 255 ? row.anns.columnLength : null);
  if ((row.type === 'string') && effLen && effLen !== 255) args.push(`length: ${effLen}`);

  // Precision / Scale (number types)
  const effPrec = row.precision !== null ? row.precision : row.anns.columnPrecision;
...
export function buildOutput({
  ...
  if (currentLang === 'ts') {
    const hasTsOrm = columnRows.some(r => {
      const effLen = r.length !== null ? r.length : (r.anns.columnLength !== 255 ? r.anns.columnLength : null);
      const effPrec = r.precision !== null ? r.precision : r.anns.columnPrecision;
      const effScal = r.scale !== null ? r.scale : r.anns.columnScale;
      return (r.type === 'string' && effLen && effLen !== 255)
    ...
    columnRows.forEach(r => {
      if (r.type === 'LocalDateTime') imports.add('import java.time.LocalDateTime;');
      ...
      const a = r.anns;
      if (a.notNull) imports.add('import jakarta.validation.constraints.NotNull;');
      if (a.notBlank) imports.add('import jakarta.validation.constraints.NotBlank;');
      if (a.sizeMin || a.sizeMax) imports.add('import jakarta.validation.constraints.Size;');
      ...
      if (a.pattern) imports.add('import jakarta.validation.constraints.Pattern;');
      if (a.relType === 'Enumerated') imports.add('import jakarta.persistence.EnumType;');
      if (a.relType === 'Enumerated') imports.add('import jakarta.persistence.Enumerated;');
      if (['OneToOne', 'ManyToOne', 'OneToMany', 'ManyToMany'].includes(a.relType)) {
    ...
    if (!columnRows.length) {
      ...
      columnRows.forEach((row, i) => {
        ...
        } else if (['OneToOne', 'ManyToOne', 'OneToMany', 'ManyToMany'].includes(a.relType)) {
          const rArgs = [`fetch = FetchType.${a.fetchType}`];
          if (a.cascade) rArgs.push(`cascade = CascadeType.${a.cascade}`);
          if (a.mappedBy && ['OneToMany', 'ManyToMany'].includes(a.relType)) rArgs.push(`mappedBy = "${a.mappedBy}"`);
        ...
        if (includeValid || includeOrm) {
          ...
          if (a.sizeMin || a.sizeMax) {
            const sArgs = [a.sizeMin ? `min = ${a.sizeMin}` : '', a.sizeMax ? `max = ${a.sizeMax}` : ''].filter(Boolean).join(', ');
        ...
        if (includeOrm) {
          if (isId) {
            ...
            if (a.generatedValue && a.generatedValue !== 'none')
              lines.push(`    @GeneratedValue(strategy = GenerationType.${a.generatedValue})`);
          }
          ...
          if (row.type === 'String' && effLen && effLen !== 255) colArgs.push(`length = ${effLen}`);
  ...
  if (currentLang === 'ts') {
    badge.textContent = 'TS'; badge.className = 'win-lang-badge lang-ts';
  } else {
    badge.textContent = 'Java'; badge.className = 'win-lang-badge lang-java';
  }
}
...
export function buildTable({ columnRows, currentLang, filterQuery }) {
  ...
  const filtered = filterQuery
    ? columnRows.filter(r => r.original.toLowerCase().includes(filterQuery) || r.camel.toLowerCase().includes(filterQuery))
    : columnRows;
  ...
  body.innerHTML = filtered.map((row, fi) => {
    ...
    if (isJava) {
      ...
      else if (a.relType !== 'none') badge = `<span class="trans-badge" style="color:var(--amber)">${a.relType.replace('ManyTo', 'N:').replace('OneToMany', '1:N').replace('OneToOne', '1:1').replace('ManyToOne', 'N:1')}</span>`;
    }

    // Determine which inline control to show based on type
    const isStringType = (currentLang === 'ts' && row.type === 'string') || (currentLang === 'java' && row.type === 'String');
    const isDecimalType = (currentLang === 'ts' && row.type === 'number') || (['BigDecimal', 'Double'].includes(row.type));
    let lenPrecCell = '';
    if (isStringType) {
...
export function updateModalPreview(idx, columnRows) {
  ...
  if (a.isTransient) {
    ...
    if (a.relType === 'Enumerated')
      ...
      if (a.mappedBy && ['OneToMany', 'ManyToMany'].includes(a.relType)) rArgs += `, mappedBy = "${a.mappedBy}"`;
      lines.push(`@${a.relType}(${rArgs})`);
    }
    if (a.notNull) lines.push('@NotNull');
    if (a.notBlank) lines.push('@NotBlank');
    if (a.sizeMin || a.sizeMax) {
      const s = [a.sizeMin ? `min = ${a.sizeMin}` : '', a.sizeMax ? `max = ${a.sizeMax}` : ''].filter(Boolean).join(', ');
      lines.push(`@Size(${s})`);
    }
    if (a.minVal) lines.push(`@Min(${a.minVal})`);
    if (a.maxVal) lines.push(`@Max(${a.maxVal})`);
    ...
    lines.push(colArgs.length === 1 ? `@Column(name = "${colName}")` : `@Column(${colArgs.join(', ')})`);
    lines.push(`private ${row.type} ${row.camel};`);
  }
...
export function openModal(idx, columnRows, activeModalTab) {
  const row = columnRows[idx];
  const a = row.anns;
  document.getElementById('modalColName').textContent = row.colName || row.original;
  document.getElementById('modalColCamel').textContent = `→ .${row.camel}  :  ${row.type}`;

  const isString = row.type === 'String';
  const isNumeric = ['Long', 'Integer', 'Double', 'BigDecimal'].includes(row.type);
  ...
  // ── Relationship Pane ──
  const REL_OPTIONS = [
    { value: 'none', icon: '⊘', label: 'None', sub: 'ไม่มี relationship', color: 'var(--text3)', dim: 'rgba(100,120,150,.08)', border: 'var(--border2)' },
    { value: 'OneToOne', icon: '⟷', label: '@OneToOne', sub: '1 Entity ↔ 1 Entity', color: 'var(--indigo)', dim: 'var(--indigo-dim)', border: 'rgba(124,106,247,.35)' },
    { value: 'ManyToOne', icon: '⟶', label: '@ManyToOne', sub: 'Many rows → 1 parent', color: 'var(--cyan)', dim: 'var(--cyan-dim)', border: 'rgba(93,216,240,.35)' },
    { value: 'OneToMany', icon: '⟵', label: '@OneToMany', sub: '1 parent → List of children', color: 'var(--amber)', dim: 'var(--amber-dim)', border: 'rgba(255,179,71,.35)' },
    { value: 'ManyToMany', icon: '⟺', label: '@ManyToMany', sub: 'List ↔ List (join table)', color: 'var(--rose)', dim: 'var(--rose-dim)', border: 'rgba(240,98,146,.35)' },
    { value: 'Enumerated', icon: '≡', label: '@Enumerated', sub: 'Java Enum mapping', color: 'var(--green-str)', dim: 'rgba(134,239,172,.08)', border: 'rgba(134,239,172,.35)' },
  ];

  function buildRelCard(opt, currentRelType) {
    const isActive = currentRelType === opt.value;
    const activeStyle = isActive
      ? 'border-color:' + opt.border + ';background:' + opt.dim + ';box-shadow:0 0 0 1px ' + opt.border + ';'
      : '';
    const labelColor = isActive ? 'color:' + opt.color : '';
    const check = isActive ? '<span class="rel-card-check">✓</span>' : '';
    return '<button class="rel-card' + (isActive ? ' rel-card-active' : '') + '" onclick="window.setRelType(\'' + opt.value + '\')"'
      + ' style="' + activeStyle + '">'
      + '<span class="rel-card-icon" style="color:' + opt.color + '">' + opt.icon + '</span>'
      + '<span class="rel-card-label" style="' + labelColor + '">' + opt.label + '</span>'
      + '<span class="rel-card-sub">' + opt.sub + '</span>'
      + check
      + '</button>';
  }

  const relCards = REL_OPTIONS.map(opt => buildRelCard(opt, a.relType)).join('');
  const isRelType = ['OneToOne', 'ManyToOne', 'OneToMany', 'ManyToMany'].includes(a.relType);
  ...
  if (isRelType) {
    ...
    const fetchHint = a.fetchType === 'LAZY' ? 'โหลดเมื่อใช้งาน (แนะนำ)' : 'โหลดพร้อม Entity';

    const cascadeOptions = [
      ['', '— none —'], ['ALL', 'ALL'], ['PERSIST', 'PERSIST'],
      ['MERGE', 'MERGE'], ['REMOVE', 'REMOVE'], ['REFRESH', 'REFRESH']
    ].map(pair => {
      return '<option value="' + pair[0] + '"' + (a.cascade === pair[0] ? ' selected' : '') + '>' + pair[1] + '</option>';
    }).join('');
    const cascadeHint = a.cascade ? '<span class="rel-opt-hint" style="color:var(--amber)">CascadeType.' + a.cascade + '</span>' : '';

    let mappedByRow = '';
    if (a.relType === 'OneToMany' || a.relType === 'ManyToMany') {
      mappedByRow = '<div class="rel-opt-row">'
        + '<div class="rel-opt-label"><span class="rel-opt-icon">↩</span><span>mappedBy</span></div>'
        + '<input class="rel-text-inp" type="text" value="' + (a.mappedBy || '') + '"'
        + ' oninput="window.setAnn(\'mappedBy\',this.value)" placeholder="ownerFieldName"/>'
        + '<span class="rel-opt-hint">field ใน owning side</span>'
        + '</div>';
    }

    relOptionsHtml = '<div class="rel-options-grid">'
      ...
      + '<button class="rel-toggle-btn' + lazyActive + '" onclick="window.setAnn(\'fetchType\',\'LAZY\')">LAZY</button>'
      + '<button class="rel-toggle-btn' + eagerActive + '" onclick="window.setAnn(\'fetchType\',\'EAGER\')">EAGER</button>'
      + '</div>'
      + '<span class="rel-opt-hint">' + fetchHint + '</span>'
      + '</div>'
      + '<div class="rel-opt-row">'
      + '<div class="rel-opt-label"><span class="rel-opt-icon">⛓</span><span>CascadeType</span></div>'
      + '<select class="rel-select" onchange="window.setAnn(\'cascade\',this.value)">' + cascadeOptions + '</select>'
      + cascadeHint
      + '</div>'
      + mappedByRow
      + '</div>';
  ...
  if (a.relType === 'Enumerated') {
    ...
    const enumHint = a.enumType === 'STRING' ? 'บันทึกชื่อ enum (แนะนำ)' : 'บันทึกเลข index';
    enumOptionsHtml = '<div class="rel-options-grid">'
      + '<div class="rel-opt-row">'
      + '<div class="rel-opt-label"><span class="rel-opt-icon">≡</span><span>EnumType</span></div>'
      + '<div class="rel-toggle-pair">'
      + '<button class="rel-toggle-btn' + strActive + '" onclick="window.setAnn(\'enumType\',\'STRING\')">STRING</button>'
      + '<button class="rel-toggle-btn' + ordActive + '" onclick="window.setAnn(\'enumType\',\'ORDINAL\')">ORDINAL</button>'
      + '</div>'
      + '<span class="rel-opt-hint">' + enumHint + '</span>'
      + '</div>'
      + '</div>';
  }
  ...
  const optionsSectionLabel = showOptions
    ? '<div class="rel-section-label" style="margin-top:4px"><span style="color:var(--amber)">⚙</span> ตั้งค่า @' + a.relType + '</div>'
    : '';
  const optionsBox = showOptions
    ? '<div class="rel-options-box">' + relOptionsHtml + enumOptionsHtml + '</div>'
    : '';

  const relPane = '<div class="modal-tab-pane" id="mpane-relationship">'
    + '<div class="rel-section-label"><span style="color:var(--amber)">⬡</span> เลือก Annotation Type</div>'
    + '<div class="rel-card-grid">' + relCards + '</div>'
    + optionsSectionLabel
    + optionsBox
    + '</div>';

  document.getElementById('modalBody').innerHTML = jpaPane + validPane + relPane;

  // Activate correct tab
  document.querySelectorAll('.modal-tab').forEach(t => t.classList.toggle('active', t.dataset.pane === activeModalTab));
  document.querySelectorAll('.modal-tab-pane').forEach(p => p.classList.toggle('active', p.id === 'mpane-' + activeModalTab));
